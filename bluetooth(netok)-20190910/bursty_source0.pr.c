/* Process model C form file: bursty_source0.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char bursty_source0_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A op_runsim 7 5D771CBB 5D771CBB 1 7FPCYKRFROZNOYF Zhi@Ren 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1d79 5                                                                                                                                                                                                                                                                                                                                                                                                ";
#include <string.h>
...
int  sent_pk_nb = 0;
Stathandle  sent_pk_nb_gshandle;
...
#define FIN_PREAMBLE_DEC	bursty_source0_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((bursty_source0_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
bursty_source_sv_init ()
	...
	if ((start_time >= 0.0) && (stop_time > start_time))
		...
		//abz
		sent_pk_nb = 0;
		sent_pk_nb_gshandle 		= op_stat_reg ("Number of Sent Pks",	OPC_STAT_INDEX_NONE, OPC_STAT_GLOBAL);
		
		dest_addr = -2;
		op_ima_obj_attr_get(op_topo_parent(my_id),"destination_address", &dest_addr);   //to get dest from attribute
...
extern "C" {
#endif
	void bursty_source0 (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_bursty_source0_init (int * init_block_ptr);
	void _op_bursty_source0_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_bursty_source0_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_bursty_source0_alloc (VosT_Obtype, int);
	void _op_bursty_source0_svar (void *, const char *, void **);
...
void
bursty_source0 (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (bursty_source0 ());
		...
		FSM_ENTER ("bursty_source0")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_UNFORCED_NOLABEL (0, "init", "bursty_source0 [init enter execs]")
				FSM_PROFILE_SECTION_IN ("bursty_source0 [init enter execs]", state0_enter_exec)
				{
			...
			FSM_EXIT (1,"bursty_source0")


			/** state (init) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "init", "bursty_source0 [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "off", "tr_0", "bursty_source0 [init -> off : default / ]")
				/*---------------------------------------------------------*/



			/** state (off) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "off", state1_enter_exec, "bursty_source0 [off enter execs]")
				FSM_PROFILE_SECTION_IN ("bursty_source0 [off enter execs]", state1_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"bursty_source0")


			/** state (off) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "off", "bursty_source0 [off exit execs]")
				FSM_PROFILE_SECTION_IN ("bursty_source0 [off exit execs]", state1_exit_exec)
				{
			...
			FSM_TRANSIT_ONLY ((INACTIVE_TO_ACTIVE), 2, state2_enter_exec, ;, off, "INACTIVE_TO_ACTIVE", "", "off", "on", "tr_1", "bursty_source0 [off -> on : INACTIVE_TO_ACTIVE / ]")
				/*---------------------------------------------------------*/



			/** state (on) enter executives **/
			FSM_STATE_ENTER_UNFORCED (2, "on", state2_enter_exec, "bursty_source0 [on enter execs]")
				FSM_PROFILE_SECTION_IN ("bursty_source0 [on enter execs]", state2_enter_exec)
				{
				...
				//	op_pk_send (pkptr, 0);
					...
					pktptr = op_pk_create_fmt("app_data");
					
					op_pk_nfd_set(pktptr, "SRC", node_addr);
					...
					//abz
					sent_pk_nb++;
					op_stat_write (sent_pk_nb_gshandle, sent_pk_nb);
					}
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"bursty_source0")


			/** state (on) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "on", "bursty_source0 [on exit execs]")
				FSM_PROFILE_SECTION_IN ("bursty_source0 [on exit execs]", state2_exit_exec)
				{
				/* Determine the type of interrupt.		*/
				intrpt_type = op_intrpt_type ();
				intrpt_code = op_intrpt_code ();
			...
			FSM_PROFILE_SECTION_IN ("bursty_source0 [on trans conditions]", state2_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ;, "ACTIVE_TO_INACTIVE", "", "on", "off", "tr_3", "bursty_source0 [on -> off : ACTIVE_TO_INACTIVE / ]")
				FSM_CASE_TRANSIT (1, 2, state2_enter_exec, ;, "REMAIN_ACTIVE", "", "on", "on", "tr_4", "bursty_source0 [on -> on : REMAIN_ACTIVE / ]")
				}
				/*---------------------------------------------------------*/
...
void
_op_bursty_source0_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_bursty_source0_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_bursty_source0_terminate ())
...
/* local variable prs_ptr in _op_bursty_source0_svar function. */
#undef pid_string
#undef debug_mode
#undef stop_time
...
VosT_Obtype
_op_bursty_source0_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_bursty_source0_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (bursty_source0)",
		sizeof (bursty_source0_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_bursty_source0_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	bursty_source0_state * ptr;
	FIN_MT (_op_bursty_source0_alloc (obtype))

	ptr = (bursty_source0_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "bursty_source0 [init enter execs]";
#endif
		}
...
void
_op_bursty_source0_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	bursty_source0_state		*prs_ptr;

	FIN_MT (_op_bursty_source0_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (bursty_source0_state *)gen_ptr;
