/* Process model C form file: l2cap.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char l2cap_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 5D727D30 5D727D30 1 USER-20151017SA Administrator 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1bcc 1                                                                                                                                                                                                                                                                                                                                                                                            ";
#include <string.h>
...
#define FIN_PREAMBLE_DEC	l2cap_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((l2cap_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
extern "C" {
#endif
	void l2cap (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_l2cap_init (int * init_block_ptr);
	void _op_l2cap_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_l2cap_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_l2cap_alloc (VosT_Obtype, int);
	void _op_l2cap_svar (void *, const char *, void **);
...
l2cap (OP_SIM_CONTEXT_ARG_OPT)
	...
	FIN_MT (l2cap ());
		...
		FSM_BLOCK_SWITCH
			...
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "l2cap [init enter execs]")
				FSM_PROFILE_SECTION_IN ("l2cap [init enter execs]", state0_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (0, "init", "l2cap [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "idle", "tr_0", "l2cap [init -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (idle) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "idle", state1_enter_exec, "l2cap [idle enter execs]")
			...
			FSM_STATE_EXIT_UNFORCED (1, "idle", "l2cap [idle exit execs]")


			/** state (idle) transition processing **/
			FSM_PROFILE_SECTION_IN ("l2cap [idle trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, ;, "UP_ARVL", "", "idle", "upper_arvl", "tr_1", "l2cap [idle -> upper_arvl : UP_ARVL / ]")
				FSM_CASE_TRANSIT (1, 3, state3_enter_exec, ;, "LOW_ARVL", "", "idle", "lower_arvl", "tr_10", "l2cap [idle -> lower_arvl : LOW_ARVL / ]")
				FSM_CASE_TRANSIT (2, 4, state4_enter_exec, ;, "SEND_BEACON", "", "idle", "send_beacon", "tr_18", "l2cap [idle -> send_beacon : SEND_BEACON / ]")
				FSM_CASE_TRANSIT (3, 5, state5_enter_exec, ;, "SEND_DATA", "", "idle", "DATA_SEND", "tr_20", "l2cap [idle -> DATA_SEND : SEND_DATA / ]")
				FSM_CASE_TRANSIT (4, 6, state6_enter_exec, ;, "SEND_CSMA", "", "idle", "bind_csma", "tr_25", "l2cap [idle -> bind_csma : SEND_CSMA / ]")
				FSM_CASE_TRANSIT (5, 1, state1_enter_exec, ;, "default", "", "idle", "idle", "tr_14", "l2cap [idle -> idle : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (2, "upper_arvl", state2_enter_exec, "l2cap [upper_arvl enter execs]")
				FSM_PROFILE_SECTION_IN ("l2cap [upper_arvl enter execs]", state2_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (2, "upper_arvl", "l2cap [upper_arvl exit execs]")


			/** state (upper_arvl) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "upper_arvl", "idle", "tr_2", "l2cap [upper_arvl -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (lower_arvl) enter executives **/
			FSM_STATE_ENTER_FORCED (3, "lower_arvl", state3_enter_exec, "l2cap [lower_arvl enter execs]")
				FSM_PROFILE_SECTION_IN ("l2cap [lower_arvl enter execs]", state3_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (3, "lower_arvl", "l2cap [lower_arvl exit execs]")


			/** state (lower_arvl) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "lower_arvl", "idle", "tr_11", "l2cap [lower_arvl -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (send_beacon) enter executives **/
			FSM_STATE_ENTER_FORCED (4, "send_beacon", state4_enter_exec, "l2cap [send_beacon enter execs]")
				FSM_PROFILE_SECTION_IN ("l2cap [send_beacon enter execs]", state4_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (4, "send_beacon", "l2cap [send_beacon exit execs]")


			/** state (send_beacon) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "send_beacon", "idle", "tr_19", "l2cap [send_beacon -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (DATA_SEND) enter executives **/
			FSM_STATE_ENTER_FORCED (5, "DATA_SEND", state5_enter_exec, "l2cap [DATA_SEND enter execs]")
				FSM_PROFILE_SECTION_IN ("l2cap [DATA_SEND enter execs]", state5_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (5, "DATA_SEND", "l2cap [DATA_SEND exit execs]")


			/** state (DATA_SEND) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "DATA_SEND", "idle", "tr_23", "l2cap [DATA_SEND -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (bind_csma) enter executives **/
			FSM_STATE_ENTER_FORCED (6, "bind_csma", state6_enter_exec, "l2cap [bind_csma enter execs]")
				FSM_PROFILE_SECTION_IN ("l2cap [bind_csma enter execs]", state6_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (6, "bind_csma", "l2cap [bind_csma exit execs]")


			/** state (bind_csma) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "bind_csma", "idle", "tr_26", "l2cap [bind_csma -> idle : default / ]")
				/*---------------------------------------------------------*/
...
void
_op_l2cap_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_l2cap_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_l2cap_terminate ())
...
/* local variable prs_ptr in _op_l2cap_svar function. */
#undef node_id
#undef listen_times
...
VosT_Obtype
_op_l2cap_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_l2cap_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (l2cap)",
		sizeof (l2cap_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_l2cap_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	l2cap_state * ptr;
	FIN_MT (_op_l2cap_alloc (obtype))

	ptr = (l2cap_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "l2cap [init enter execs]";
#endif
		}
...
void
_op_l2cap_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	l2cap_state		*prs_ptr;

	FIN_MT (_op_l2cap_svar (gen_ptr, var_name, var_p_ptr))
