/* Process model C form file: bt_source.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char bt_source_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A op_runsim 7 5D770049 5D770049 1 USER-20151017SA Administrator 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1bcc 1                                                                                                                                                                                                                                                                                                                                                                                          ";
#include <string.h>
...
#define FIN_PREAMBLE_DEC	bt_source_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((bt_source_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
extern "C" {
#endif
	void bt_source (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_bt_source_init (int * init_block_ptr);
	void _op_bt_source_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_bt_source_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_bt_source_alloc (VosT_Obtype, int);
	void _op_bt_source_svar (void *, const char *, void **);
...
void
bt_source (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	...
	FIN_MT (bt_source ());
		...
		FSM_BLOCK_SWITCH
			...
			FSM_STATE_ENTER_UNFORCED_NOLABEL (0, "init", "bt_source [init enter execs]")
				FSM_PROFILE_SECTION_IN ("bt_source [init enter execs]", state0_enter_exec)
				{
			...
			FSM_EXIT (1,"bt_source")


			/** state (init) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "init", "bt_source [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "off", "tr_0", "bt_source [init -> off : default / ]")
				/*---------------------------------------------------------*/



			/** state (off) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "off", state1_enter_exec, "bt_source [off enter execs]")
				FSM_PROFILE_SECTION_IN ("bt_source [off enter execs]", state1_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"bt_source")


			/** state (off) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "off", "bt_source [off exit execs]")
				FSM_PROFILE_SECTION_IN ("bt_source [off exit execs]", state1_exit_exec)
				{
			...
			FSM_TRANSIT_ONLY ((INACTIVE_TO_ACTIVE), 2, state2_enter_exec, ;, off, "INACTIVE_TO_ACTIVE", "", "off", "on", "tr_1", "bt_source [off -> on : INACTIVE_TO_ACTIVE / ]")
				/*---------------------------------------------------------*/



			/** state (on) enter executives **/
			FSM_STATE_ENTER_UNFORCED (2, "on", state2_enter_exec, "bt_source [on enter execs]")
				FSM_PROFILE_SECTION_IN ("bt_source [on enter execs]", state2_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"bt_source")


			/** state (on) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "on", "bt_source [on exit execs]")
				FSM_PROFILE_SECTION_IN ("bt_source [on exit execs]", state2_exit_exec)
				{
				/* Determine the type of interrupt.		*/
				intrpt_type = op_intrpt_type ();
				intrpt_code = op_intrpt_code ();
			...
			FSM_PROFILE_SECTION_IN ("bt_source [on trans conditions]", state2_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ;, "ACTIVE_TO_INACTIVE", "", "on", "off", "tr_3", "bt_source [on -> off : ACTIVE_TO_INACTIVE / ]")
				FSM_CASE_TRANSIT (1, 2, state2_enter_exec, ;, "REMAIN_ACTIVE", "", "on", "on", "tr_4", "bt_source [on -> on : REMAIN_ACTIVE / ]")
				}
				/*---------------------------------------------------------*/
...
void
_op_bt_source_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_bt_source_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_bt_source_terminate ())
...
/* local variable prs_ptr in _op_bt_source_svar function. */
#undef pid_string
#undef debug_mode
#undef stop_time
...
VosT_Obtype
_op_bt_source_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_bt_source_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (bt_source)",
		sizeof (bt_source_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_bt_source_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	bt_source_state * ptr;
	FIN_MT (_op_bt_source_alloc (obtype))

	ptr = (bt_source_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "bt_source [init enter execs]";
#endif
		}
...
void
_op_bt_source_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	bt_source_state		*prs_ptr;

	FIN_MT (_op_bt_source_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (bt_source_state *)gen_ptr;
