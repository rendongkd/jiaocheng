/* Process model C form file: bursty_source_blue_tooth.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char bursty_source_blue_tooth_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A op_runsim 7 5D70CC58 5D70CC58 1 USER-20151017SA Administrator 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1bcc 1                                                                                                                                                                                                                                                                                                                                                                                          ";
#include <string.h>
...
typedef struct
	...
	Stathandle	             		sent_pk_nb_gstathandle                          ;
	} bursty_source_blue_tooth_state;
...
#define sent_pk_nb_gstathandle  		op_sv_ptr->sent_pk_nb_gstathandle
...
#define FIN_PREAMBLE_DEC	bursty_source_blue_tooth_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((bursty_source_blue_tooth_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
bursty_source_sv_init ()
	...
	if ((start_time >= 0.0) && (stop_time > start_time))
		...
		sent_pk_nb_gstathandle 		= op_stat_reg ("Sent Packet Number",	OPC_STAT_INDEX_NONE, OPC_STAT_GLOBAL);
...
extern "C" {
#endif
	void bursty_source_blue_tooth (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_bursty_source_blue_tooth_init (int * init_block_ptr);
	void _op_bursty_source_blue_tooth_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_bursty_source_blue_tooth_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_bursty_source_blue_tooth_alloc (VosT_Obtype, int);
	void _op_bursty_source_blue_tooth_svar (void *, const char *, void **);
...
void
bursty_source_blue_tooth (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (bursty_source_blue_tooth ());
		...
		FSM_ENTER ("bursty_source_blue_tooth")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_UNFORCED_NOLABEL (0, "init", "bursty_source_blue_tooth [init enter execs]")
				FSM_PROFILE_SECTION_IN ("bursty_source_blue_tooth [init enter execs]", state0_enter_exec)
				{
			...
			FSM_EXIT (1,"bursty_source_blue_tooth")


			/** state (init) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "init", "bursty_source_blue_tooth [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "off", "tr_0", "bursty_source_blue_tooth [init -> off : default / ]")
				/*---------------------------------------------------------*/



			/** state (off) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "off", state1_enter_exec, "bursty_source_blue_tooth [off enter execs]")
				FSM_PROFILE_SECTION_IN ("bursty_source_blue_tooth [off enter execs]", state1_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"bursty_source_blue_tooth")


			/** state (off) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "off", "bursty_source_blue_tooth [off exit execs]")
				FSM_PROFILE_SECTION_IN ("bursty_source_blue_tooth [off exit execs]", state1_exit_exec)
				{
			...
			FSM_TRANSIT_ONLY ((INACTIVE_TO_ACTIVE), 2, state2_enter_exec, ;, off, "INACTIVE_TO_ACTIVE", "", "off", "on", "tr_1", "bursty_source_blue_tooth [off -> on : INACTIVE_TO_ACTIVE / ]")
				/*---------------------------------------------------------*/



			/** state (on) enter executives **/
			FSM_STATE_ENTER_UNFORCED (2, "on", state2_enter_exec, "bursty_source_blue_tooth [on enter execs]")
				FSM_PROFILE_SECTION_IN ("bursty_source_blue_tooth [on enter execs]", state2_enter_exec)
				{
				...
				if (segmentation_size > 0  && pksize > segmentation_size)
				   ...
				   while (pksize > 0)
					...
					//z add
					sent_pk_nb++;
					op_stat_write (sent_pk_nb_gstathandle, sent_pk_nb); 
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"bursty_source_blue_tooth")


			/** state (on) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "on", "bursty_source_blue_tooth [on exit execs]")
				FSM_PROFILE_SECTION_IN ("bursty_source_blue_tooth [on exit execs]", state2_exit_exec)
				{
				/* Determine the type of interrupt.		*/
				intrpt_type = op_intrpt_type ();
				intrpt_code = op_intrpt_code ();
			...
			FSM_PROFILE_SECTION_IN ("bursty_source_blue_tooth [on trans conditions]", state2_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ;, "ACTIVE_TO_INACTIVE", "", "on", "off", "tr_3", "bursty_source_blue_tooth [on -> off : ACTIVE_TO_INACTIVE / ]")
				FSM_CASE_TRANSIT (1, 2, state2_enter_exec, ;, "REMAIN_ACTIVE", "", "on", "on", "tr_4", "bursty_source_blue_tooth [on -> on : REMAIN_ACTIVE / ]")
				}
				/*---------------------------------------------------------*/
		...
		FSM_EXIT (0,"bursty_source_blue_tooth")
		}
	}
...
void
_op_bursty_source_blue_tooth_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_bursty_source_blue_tooth_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_bursty_source_blue_tooth_terminate ())
...
/* local variable prs_ptr in _op_bursty_source_blue_tooth_svar function. */
#undef pid_string
#undef debug_mode
#undef stop_time
...
VosT_Obtype
_op_bursty_source_blue_tooth_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_bursty_source_blue_tooth_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (bursty_source_blue_tooth)",
		sizeof (bursty_source_blue_tooth_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_bursty_source_blue_tooth_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	bursty_source_blue_tooth_state * ptr;
	FIN_MT (_op_bursty_source_blue_tooth_alloc (obtype))

	ptr = (bursty_source_blue_tooth_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "bursty_source_blue_tooth [init enter execs]";
#endif
		}
...
void
_op_bursty_source_blue_tooth_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	bursty_source_blue_tooth_state		*prs_ptr;

	FIN_MT (_op_bursty_source_blue_tooth_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (bursty_source_blue_tooth_state *)gen_ptr;
	...
	if (strcmp ("sent_pk_nb_gstathandle" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->sent_pk_nb_gstathandle);
		FOUT
		}
	*var_p_ptr = (void *)OPC_NIL;
