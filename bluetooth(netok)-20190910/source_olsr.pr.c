/* Process model C form file: source_olsr.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char source_olsr_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A op_runsim 7 5DBA42E3 5DBA42E3 1 7FPCYKRFROZNOYF Zhi@Ren 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1d79 5                                                                                                                                                                                                                                                                                                                                                                                                ";
#include <string.h>
...
#define destination             		op_sv_ptr->destination
...
#define FIN_PREAMBLE_DEC	source_olsr_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((source_olsr_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
bursty_source_sv_init ()
	...
	my_address = 0;
	op_ima_obj_attr_get (op_id_parent(my_id), "Node Address", &my_address);

	destination = 0;
	op_ima_obj_attr_get (my_id, "Destination", &destination);
...
extern "C" {
#endif
	void source_olsr (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_source_olsr_init (int * init_block_ptr);
	void _op_source_olsr_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_source_olsr_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_source_olsr_alloc (VosT_Obtype, int);
	void _op_source_olsr_svar (void *, const char *, void **);
...
void
source_olsr (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	...
	FIN_MT (source_olsr ());
		...
		FSM_ENTER ("source_olsr")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_UNFORCED_NOLABEL (0, "init", "source_olsr [init enter execs]")
				FSM_PROFILE_SECTION_IN ("source_olsr [init enter execs]", state0_enter_exec)
				{
			...
			FSM_EXIT (1,"source_olsr")


			/** state (init) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "init", "source_olsr [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "off", "tr_0", "source_olsr [init -> off : default / ]")
				/*---------------------------------------------------------*/



			/** state (off) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "off", state1_enter_exec, "source_olsr [off enter execs]")
				FSM_PROFILE_SECTION_IN ("source_olsr [off enter execs]", state1_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"source_olsr")


			/** state (off) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "off", "source_olsr [off exit execs]")
				FSM_PROFILE_SECTION_IN ("source_olsr [off exit execs]", state1_exit_exec)
				{
			...
			FSM_TRANSIT_ONLY ((INACTIVE_TO_ACTIVE), 2, state2_enter_exec, ;, off, "INACTIVE_TO_ACTIVE", "", "off", "on", "tr_1", "source_olsr [off -> on : INACTIVE_TO_ACTIVE / ]")
				/*---------------------------------------------------------*/



			/** state (on) enter executives **/
			FSM_STATE_ENTER_UNFORCED (2, "on", state2_enter_exec, "source_olsr [on enter execs]")
				FSM_PROFILE_SECTION_IN ("source_olsr [on enter execs]", state2_enter_exec)
				{
				...
				pksize *= 8;
				pkptr1  = op_pk_create (pksize);
				
				//create a formatted data pk
				pkptr = op_pk_create_fmt("Data");
				priority=(int)op_dist_uniform(8);
				
				//set fields of data pk
				op_pk_nfd_set(pkptr, "priority", priority);
				op_pk_nfd_set(pkptr, "Type", data_type);
				op_pk_nfd_set(pkptr, "Source Address", my_address);
				op_pk_nfd_set(pkptr, "Destination Address", destination);
				op_pk_nfd_set(pkptr, "Data", pkptr1);
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"source_olsr")


			/** state (on) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "on", "source_olsr [on exit execs]")
				FSM_PROFILE_SECTION_IN ("source_olsr [on exit execs]", state2_exit_exec)
				{
				/* Determine the type of interrupt.		*/
				intrpt_type = op_intrpt_type ();
				intrpt_code = op_intrpt_code ();
			...
			FSM_PROFILE_SECTION_IN ("source_olsr [on trans conditions]", state2_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ;, "ACTIVE_TO_INACTIVE", "", "on", "off", "tr_3", "source_olsr [on -> off : ACTIVE_TO_INACTIVE / ]")
				FSM_CASE_TRANSIT (1, 2, state2_enter_exec, ;, "REMAIN_ACTIVE", "", "on", "on", "tr_4", "source_olsr [on -> on : REMAIN_ACTIVE / ]")
				}
				/*---------------------------------------------------------*/
...
void
_op_source_olsr_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_source_olsr_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_source_olsr_terminate ())
...
/* local variable prs_ptr in _op_source_olsr_svar function. */
#undef pid_string
#undef debug_mode
#undef stop_time
...
VosT_Obtype
_op_source_olsr_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_source_olsr_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (source_olsr)",
		sizeof (source_olsr_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_source_olsr_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	source_olsr_state * ptr;
	FIN_MT (_op_source_olsr_alloc (obtype))

	ptr = (source_olsr_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "source_olsr [init enter execs]";
#endif
		}
...
void
_op_source_olsr_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	source_olsr_state		*prs_ptr;

	FIN_MT (_op_source_olsr_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (source_olsr_state *)gen_ptr;
	...
	if (strcmp ("destination" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->destination);
		FOUT
		}
